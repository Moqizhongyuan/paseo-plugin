...
2. 使用真实 PPE 环境
必须通过 GPT 浏览器插件提供的浏览器操作入口（如当前可用的 cua_repl 浏览器工具）连接我日常使用且已登录的浏览器。所有浏览器操作都必须走 GPT 插件，禁止使用 CDP、CDP Proxy、远程调试端口或通过 web-access Skill 启动 CDP，也不得在插件不可用时自动回退到 CDP。若 GPT 插件入口不可用或未连接，报告阻塞并等待我处理。确认页面命中本次 PPE 和对应代码版本，不得用本地开发环境、其他泳道或其他版本替代。
使用 GPT 插件提供的桌面级 Computer Use（桌面截图、鼠标和键盘操作）打开 Chrome 工具栏中的 ENV 插件，为本次测试新建专用标签页和独立配置组，配置从 MR 或关联 Bits 开发任务确认的 PPE。必须先启用 Tab Filter 并核对其绑定的是本次测试标签页，再启用 PPE 配置；不能仅用 Tab Domain Filter 代替标签页隔离。记录原有配置和本次新增配置，不得覆盖用户已有配置、改变其开关或影响其他标签页。如果已有配置冲突且无法在不改动用户配置的情况下隔离，报告阻塞并等待我处理。若 GPT 插件不具备桌面级 Computer Use 能力、无法操作浏览器工具栏或扩展弹窗，或 ENV 插件不可用，报告阻塞并等待我处理，不得回退到 CDP。配置后刷新测试页面，核验请求实际命中目标 PPE，并单独核验前端代码版本；不能仅凭请求头已添加就认定环境正确。
...
补测开始前，通过 GPT 插件提供的网络查看能力，或通过桌面级 Computer Use 打开测试标签页开发者工具的 Network 面板，保留本批操作前后的请求记录。根据实际请求、插桩配置或源码确认华佗覆盖率上报的真实域名、路径和请求特征，再筛选上报请求；不得猜测固定接口，也不要把普通业务埋点当成覆盖率上报。这里的过滤仅用于查看，不得拦截、修改或重放请求，禁止改走 CDP。若无法查看必要的上报请求，报告阻塞，不得宣称已确认上报成功。
每批操作后，检查对应上报请求的发起时间、完成状态、HTTP 状态码，以及可获取的响应体、业务状态和错误信息；结合实际接口语义判断是否上报成功，不能仅凭出现请求或 HTTP 2xx 就认定成功。区分未观察到请求、请求仍在等待、明确上报失败、上报成功但报告尚未更新、报告已确认覆盖这几种状态；保留脱敏证据，记录可观察到的自动重试及结果。发生明确上报失败时暂停后续补测，保留现场并报告，等待我决定，不自行修复或手工补发。
考虑上报和报告处理可能延迟：优先按平台说明的上报周期和处理时延分段等待、回读；没有说明时，每 30 秒检查一次，单批最多等待 5 分钟。不要为催促上报而重复执行有数据副作用的操作。等待超时仍无请求、请求未完成或报告未更新时，说明当前状态及证据并报告阻塞，不把“尚未回收”当成“代码未执行”，也不无限等待。只有华佗报告回读确认后才计入新增覆盖；关闭测试标签页或清理 ENV 配置前，先完成本批上报状态核验。
...
正常收尾时，先停用并删除本次新增的 ENV 配置组，核对用户原有配置未变，再关闭测试标签页，并汇报 ENV 配置清理情况。若因异常暂停，优先遵守第 5 条保留现场，未经确认不清理会破坏异常证据的配置或标签页。
汇报中补充上报请求的成功、失败或等待状态、报告回读时间和延迟情况；正常收尾时恢复本次修改的 Network 过滤和日志保留等设置。
