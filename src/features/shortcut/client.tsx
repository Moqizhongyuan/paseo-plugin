...
通过 GPT 浏览器插件为本次测试创建专用标签页，创建后立即暂停并结束本轮回复，保留该标签页，告知我标签页地址和从 MR 或关联 Bits 开发任务确认的目标 PPE，由我手动配置 PPE 和 Tab Filter。Agent 不操作 ENV 插件，不新增、修改或清理 ENV 配置。必须等我明确回复配置完成并继续后，才能刷新测试页面、核验环境和开始补测；不得在等待期间自行继续。继续后核验请求实际命中目标 PPE，并单独核验前端代码版本；不能仅凭请求头已添加就认定环境正确。
...
ENV 配置由我手动管理，正常收尾时提醒我清理本次 PPE 和 Tab Filter，Agent 不操作这些配置。若因异常暂停，优先遵守第 5 条保留现场，未经确认不清理会破坏异常证据的配置或标签页。
汇报中补充上报请求的成功、失败或等待状态、报告回读时间和延迟情况；正常收尾时恢复本次修改的 Network 过滤和日志保留等设置。

除创建测试标签页后等待我手动配置 PPE 和 Tab Filter、目标信息无法确定、缺少必要权限或测试数据、发现异常及需要高风险操作授权外，连续执行，不逐步要求我确认。`;
}
