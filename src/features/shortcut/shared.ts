...
const meegoUrl = z.string().trim().min(1).max(2048);

export const DEFAULT_MR_URL = "默认链接";
export const DEFAULT_MEEGO_URL = "默认链接";
...
export const getShortcutBinding = defineRpc({
  ...
  output: z.object({
    ...
    meegoUrl: z.string().nullable(),
  }),
});

export const saveShortcutBinding = defineRpc({
  name: "shortcut.binding.save",
  input: z.object({ agentId, branch, mrUrl, meegoUrl }),
  output: z.object({ branch: z.string(), mrUrl: z.string(), meegoUrl: z.string() }),
});
