...
test("Huatuo command creates one agent in the current workspace with the bound MR and safety requirements", async () => {
  ...
  for (const requirement of [
    ...
    "使用 GPT 插件提供的桌面级 Computer Use",
    "打开 Chrome 工具栏中的 ENV 插件",
    "新建专用标签页和独立配置组",
    "必须先启用 Tab Filter 并核对其绑定的是本次测试标签页，再启用 PPE 配置",
    "不能仅用 Tab Domain Filter 代替标签页隔离",
    "不得覆盖用户已有配置、改变其开关或影响其他标签页",
    "如果已有配置冲突且无法在不改动用户配置的情况下隔离，报告阻塞并等待我处理",
    "无法操作浏览器工具栏或扩展弹窗，或 ENV 插件不可用，报告阻塞并等待我处理，不得回退到 CDP",
    "核验请求实际命中目标 PPE，并单独核验前端代码版本",
    "先停用并删除本次新增的 ENV 配置组，核对用户原有配置未变，再关闭测试标签页",
