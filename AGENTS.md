...
- `contract.ts`：客户端与服务端共用的 RPC 契约、Zod schema 和类型。
...
- `client.tsx` 可以依赖 `contract.ts`，但不能导入 Node.js 模块或服务端实现。
- `server.ts` 可以依赖 `contract.ts` 以及 Feature 内的 service/repository 模块，但不能导入 React Native UI。
- `contract.ts` 不能依赖客户端或服务端实现。
